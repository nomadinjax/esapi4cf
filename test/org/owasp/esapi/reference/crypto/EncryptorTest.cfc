...
<cfcomponent extends="cfesapi.test.org.owasp.esapi.lang.TestCase" output="false">
	...
	<cffunction access="public" returntype="void" name="testEncryptEmptyStrings" output="false" hint="Test of deprecated encrypt methods for empty String.">
		<cfscript>
	        ...
	        //try {
	            ...
	            local.ct = local.encryptor.encrypt(plain=new cfesapi.org.owasp.esapi.crypto.PlainText(instance.ESAPI, local.plaintext));
	...
	<cffunction access="public" returntype="void" name="testSeal" output="false" hint="Test of seal method, of class org.owasp.esapi.Encryptor.">
		<cfscript>
	        ...
	        local.encryptor = instance.ESAPI.encryptor();
	        local.plaintext = instance.ESAPI.randomizer().getRandomString( 32, newJava("org.owasp.esapi.reference.DefaultEncoder").CHAR_ALPHANUMERICS );
	        local.seal = local.encryptor.seal( local.plaintext, newJava("java.lang.Long").init(local.encryptor.getTimeStamp() + 1000*60).longValue() );
	        local.encryptor.verifySeal( local.seal );

	        local.progressMark = 1;
	        local.caughtExpectedEx = false;
	        try {
	            local.seal = local.encryptor.seal("", newJava("java.lang.Long").init(local.encryptor.getTimeStamp() + 1000*60).longValue());
	            local.progressMark++;
	            local.encryptor.verifySeal(local.seal);
	            local.progressMark++;
	        } catch(java.lang.Exception e) {
	        ...
	        /* NULL test
	        try {
	            local.seal = local.encryptor.seal(null, newJava("java.lang.Long").init(local.encryptor.getTimeStamp() + 1000*60).longValue());
	...
	<cffunction access="public" returntype="void" name="testVerifySeal" output="false" hint="Test of verifySeal method, of class org.owasp.esapi.Encryptor.">
		<cfscript>
	        ...
	        try {
	        	...
	        	local.progressMark++;
	            assertFalse( local.encryptor.verifySeal( local.encryptor.encrypt( plain=new cfesapi.org.owasp.esapi.crypto.PlainText(instance.ESAPI, local.plaintext) ).getBase64EncodedRawCipherText() ) );
	            local.progressMark++;
	            assertFalse( local.encryptor.verifySeal( local.encryptor.encrypt( plain=new cfesapi.org.owasp.esapi.crypto.PlainText(instance.ESAPI, 100 & ":" & local.plaintext) ).getBase64EncodedRawCipherText() ) );
	            local.progressMark++;
	            assertFalse( local.encryptor.verifySeal( local.encryptor.encrypt( plain=new cfesapi.org.owasp.esapi.crypto.PlainText(instance.ESAPI, newJava("java.lang.Long").MAX_VALUE & ":" & local.plaintext) ).getBase64EncodedRawCipherText() ) );
	            local.progressMark++;
	            assertFalse( local.encryptor.verifySeal( local.encryptor.encrypt( plain=new cfesapi.org.owasp.esapi.crypto.PlainText(instance.ESAPI, newJava("java.lang.Long").MAX_VALUE & ":random:" & local.plaintext) ).getBase64EncodedRawCipherText() ) );
	            local.progressMark++;
	            assertFalse( local.encryptor.verifySeal( local.encryptor.encrypt( plain=new cfesapi.org.owasp.esapi.crypto.PlainText(instance.ESAPI, newJava("java.lang.Long").MAX_VALUE & ":random:" & local.plaintext & ":badsig")  ).getBase64EncodedRawCipherText() ) );
	            local.progressMark++;
	            assertFalse( local.encryptor.verifySeal( local.encryptor.encrypt( plain=new cfesapi.org.owasp.esapi.crypto.PlainText(instance.ESAPI, newJava("java.lang.Long").MAX_VALUE & ":random:" & local.plaintext & ":" & local.encryptor.sign( newJava("java.lang.Long").MAX_VALUE & ":random:" & local.plaintext) ) ).getBase64EncodedRawCipherText() ) );
	            local.progressMark++;
	        } catch ( Exception e ) {
	...
	<cffunction access="public" returntype="void" name="testEncryptionSerialization" output="false">
		<cfscript>
	        ...
	        instance.ESAPI.securityConfiguration().setCipherTransformation("AES/CBC/PKCS5Padding");
	        local.ct = instance.ESAPI.encryptor().encrypt(plain=new cfesapi.org.owasp.esapi.crypto.PlainText(instance.ESAPI, local.secretMsg));
