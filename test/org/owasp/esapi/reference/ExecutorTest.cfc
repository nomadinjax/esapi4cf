...
<cfcomponent extends="esapi4cf.test.org.owasp.esapi.util.TestCase" output="false">
	...
	<cffunction access="public" returntype="void" name="testExecuteUnixSystemCommand" output="false">
		<cfscript>
			...
			variables.ESAPI.setSecurityConfiguration(
					createObject("component", "ExecutorTest$Conf").init(
						variables.ESAPI.securityConfiguration(),
						newJava("java.util.Collections").singletonList(binSh.getPath()),
						workingDir
						)
					);
	...
	<cffunction access="public" returntype="void" name="testExecuteSystemCommand" output="false">
		<cfscript>
			...
			variables.ESAPI.setSecurityConfiguration(
					createObject("component", "ExecutorTest$Conf").init(
						variables.ESAPI.securityConfiguration(),
						newJava("java.util.Collections").singletonList(javaHomeBinJava.getPath()),
						tmpDir
						)
					);
