...
<cfcomponent extends="esapi4cf-test.org.owasp.esapi.util.TestCase" output="false">
	...
	<cffunction access="private" returntype="esapi4cf.org.owasp.esapi.reference.DefaultUser" name="createTestUser" output="false"
		...
		<cfscript>
			...
			local.e = getJava( "java.lang.Exception" ).init();
			local.st = local.e.getStackTrace();
			System.out.println( "Creating user " & local.username & " for " & local.st[1].getMethodName() );
	...
	<cffunction access="public" type="void" name="testAddRole" output="false"
		...
		<cfscript>
			...
			local.role = instance.ESAPI.randomizer().getRandomString( 8, getJava( "org.owasp.esapi.reference.DefaultEncoder" ).CHAR_LOWERS );
	...
	<cffunction access="public" type="void" name="testChangePassword" output="false"
		...
		<cfscript>
			...
			local.user = createTestUser( local.oldPassword );
			System.out.println( "Hash of " & local.oldPassword & " = " & local.authenticator.getHashedPassword( local.user ) );
			local.password1 = "SomethingElse34##$";
			local.user.changePassword( local.oldPassword, local.password1, local.password1 );
			System.out.println( "Hash of " & local.password1 & " = " & local.authenticator.getHashedPassword( local.user ) );
			...
			System.out.println( "Hash of " & local.password2 & " = " & local.authenticator.getHashedPassword( local.user ) );
			try {
	...
	<cffunction access="public" type="void" name="testGetAccountName" output="false"
		...
		<cfscript>
			...
			local.accountName = instance.ESAPI.randomizer().getRandomString( 7, getJava( "org.owasp.esapi.reference.DefaultEncoder" ).CHAR_ALPHANUMERICS );
	...
	<cffunction access="public" type="void" name="testGetScreenName" output="false"
		...
		<cfscript>
			...
			local.screenName = instance.ESAPI.randomizer().getRandomString( 7, getJava( "org.owasp.esapi.reference.DefaultEncoder" ).CHAR_ALPHANUMERICS );
	...
	<cffunction access="public" type="void" name="testIsSessionAbsoluteTimeout" output="false"
		...
		<cfscript>
			...
			local.session.setCreationTime( javaCast( "long", local.now - (1000 * 60 * 60 * 3) ) );
			assertTrue( local.user.isSessionAbsoluteTimeout() );

			// set session creation -1 hour (default is 2 hour timeout)
			local.session.setCreationTime( javaCast( "long", local.now - (1000 * 60 * 60 * 1) ) );
	...
	<cffunction access="public" type="void" name="testSetExpirationTime" output="false"
		...
		<cfscript>
			...
			System.out.println( "setAccountName" );
			local.password = instance.ESAPI.randomizer().getRandomString( 8, getJava( "org.owasp.esapi.reference.DefaultEncoder" ).CHAR_ALPHANUMERICS );
			local.user = createTestUser( local.password );
			local.user.setExpirationTime( getJava( "java.util.Date" ).init( javaCast( "long", 0 ) ) );
