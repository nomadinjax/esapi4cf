...
<cfscript>
	...
	} catch (esapi4cf.org.owasp.esapi.errors.IntrusionException ie) {
		input = "double encoding attack detected";
		ESAPI().currentRequest().setAttribute("userMessage", ie.message);
		ESAPI().currentRequest().setAttribute("logMessage", ie.detail);
	} catch (java.lang.Exception e) {
		input = "exception thrown";
		ESAPI().currentRequest().setAttribute("logMessage", e.message);
	}
</cfscript>
