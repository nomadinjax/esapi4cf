<cfoutput>
<script type="text/javascript" src="/SwingSet/javascript/jquery-1.2.6.js"></script>
...
<b><a href="main?function=Randomizer&solution">Solution</a></b> |
...
<h2 text align=center>Randomizer Solution</h2>
<h4>JSP Location: WebContent\WEB-INF\jsp\RandomizerSolution.jsp</h4>
<p>The form below contains ajax calls which invoke the methods of the class org.owasp.esapi.swingset.actions.RandomizerSolution to create the random data types.</p>

<p><b>Java Location: Java Resources:src/org.owasp.esapi.swingset.actions.RandomizerSolution.java</b></p>

<form name="secureDemo" action="?function=Randomizer&solution" method="post">

	<div>
		<h4>Generate a random boolean</h4>
		Random Boolean: <font color="green"><span id="randomBoolean"></span></font>
		<br /><br />
		<input type="button" value="Get Random Boolean" onclick="sendAjaxRequest('/SwingSet/ajax?function=Randomizer&solution&method=getRandomBoolean&timestamp='+timestamp(),'booleanChanged')" />
	</div>
	<div>
		...
		<input type="button" value="Submit" onclick="sendAjaxRequest('/SwingSet/ajax?function=Randomizer&solution&method=getRandomFileName&fileExtension=' + document.secureDemo.fileExtension.value + '&timestamp=' + timestamp(),'randomFileNameChanged')">
	...
	<div>
		...
		<input type="button" value="Submit" onclick="sendAjaxRequest('/SwingSet/ajax?function=Randomizer&solution&method=getRandomInteger&min=' + document.secureDemo.min.value + '&max=' + document.secureDemo.max.value + '&timestamp=' + timestamp(),'randomIntegerChanged')">
	...
	<div>
		...
		<br/><br/><input type="button" value="Submit" onclick="sendAjaxRequest('ajax?function=Randomizer&solution&method=getRandomLong&timestamp=' + timestamp(),'randomLongChanged')">
	</div>
	<div>
		...
		<input type="button" value="Submit" onclick="sendAjaxRequest('ajax?function=Randomizer&solution&method=getRandomReal&minFloat=' + document.secureDemo.minFloat.value + '&maxFloat=' + document.secureDemo.maxFloat.value + '&timestamp=' + timestamp(),'randomRealChanged')">
	...
	<div>
		...
		<input type="button" value="Submit" onclick="sendAjaxRequest('ajax?function=Randomizer&solution&method=getRandomString&length=' + document.secureDemo.length.value + '&charSet=' + document.secureDemo.charSet.value + '&timestamp=' + timestamp(),'randomStringChanged')">
