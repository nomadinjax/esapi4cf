...
<cftry>
	<cfif ESAPI().currentRequest().getParameter("create_username") NEQ "">
		<!--- TODO 1: Use ESAPI to Create a User Account --->
		<cfoutput>User Created : #ESAPI().currentRequest().getParameter("create_username")#</cfoutput>
	<cfelse>
		<cfset user = ESAPI().authenticator().login(ESAPI().currentRequest(), ESAPI().currentResponse())>
		<!--- TODO 2: Login using ESAPI --->
		<cfif user NEQ "">
			<cfif ESAPI().currentRequest().getParameter("remember") NEQ "">
				<cfset ESAPI().httpUtilities().setRememberToken(ESAPI().currentRequest().getParameter("password"), 8000, "", "") />
			</cfif>
			Current User:
