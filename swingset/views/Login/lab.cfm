...
<cftry>
	...
	<cfelse>
		...
		<cfif user NEQ "">
			<cfif form.remember NEQ "">
				<cfset application.ESAPI.httpUtilities().setRememberToken(form.password, 8000, "", "") />
			</cfif>
			Current User:
			...
			Current Cookies:
			<br />
			<cfset cookies = application.ESAPI.httpUtilities().getCurrentRequest().getCookies()/>
	...
	<cfcatch type="java.lang.Exception">
		<cfset application.ESAPI.currentRequest().setAttribute("userMessage", "there was a problem..." )>
		<cfset application.ESAPI.currentRequest().setAttribute("userMessage", cfcatch.message)>
		<cfset cfcatch.printStackTrace()>
	</cfcatch>
</cftry>
