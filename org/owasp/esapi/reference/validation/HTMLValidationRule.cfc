...
<cfcomponent extends="StringValidationRule" output="false">
	...
	<cffunction access="public" returntype="HTMLValidationRule" name="init" output="false">
		...
		<cfscript>
			...
			if(structKeyExists(local, "resourceStream")) {
				try {
	...
	<cffunction access="private" returntype="String" name="invokeAntiSamy" output="false">
		...
		<cfscript>
			...
			if(newJava("org.owasp.esapi.StringUtilities").isEmpty(arguments.input)) {
				...
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.ValidationException").init(ESAPI=instance.ESAPI, userMessage=arguments.context & " is required", logMessage="AntiSamy validation error: context=" & arguments.context & ", input=" & arguments.input, context=arguments.context));
			}
			...
			try {
				...
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.ValidationException").init(instance.ESAPI, arguments.context & ": Invalid HTML input", "Invalid HTML input: context=" & arguments.context & " error=" & e.message, e, arguments.context));
			}
			catch(org.owasp.validator.html.PolicyException e) {
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.ValidationException").init(instance.ESAPI, arguments.context & ": Invalid HTML input", "Invalid HTML input does not follow rules in antisamy-esapi.xml: context=" & arguments.context & " error=" & e.message, e, arguments.context));
			}
		</cfscript>
