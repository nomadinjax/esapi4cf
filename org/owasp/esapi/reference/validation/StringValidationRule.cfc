...
<cfcomponent extends="BaseValidationRule" output="false">
	...
	<cffunction access="public" returntype="void" name="setValidateInputAndCanonical" output="false"
	            hint="Set the flag which determines whether the in input itself is checked as well as the canonical form of the input.">
	...
	<cffunction access="private" returntype="String" name="checkWhitelist" output="false"
	            hint="checks input against whitelists.">
		...
		<cfargument type="String" name="orig" required="false" default="#arguments.input#"
		            hint="A origional input to include in exception messages. This is not included if it is the same as input."/>
		...
		<cfscript>
			...
			for(local.i = 1; local.i <= arrayLen(instance.whitelistPatterns); local.i++) {
				...
				if(!local.p.matcher(arguments.input).matches()) {
					...
					throwError(newComponent("cfesapi.org.owasp.esapi.errors.ValidationException").init(ESAPI=instance.ESAPI, userMessage=arguments.context & ": Invalid input. Please conform to regex " & local.p.pattern() & (iif(instance.maxLength == newJava("java.lang.Integer").MAX_VALUE, de(""), de(" with a maximum length of " & instance.maxLength))), logMessage="Invalid input: context=" & arguments.context & ", type(" & getTypeName() & ")=" & local.p.pattern() & ", input=" & arguments.input & iif(NullSafe.equals(arguments.orig, arguments.input), de(""), de(", orig=" & arguments.orig)), context=arguments.context));
				}
			}
	...
	<cffunction access="private" returntype="String" name="checkBlacklist" output="false"
	            hint="checks input against blacklists.">
		<cfargument type="String" name="context" required="true" hint="The context to include in exception messages"/>
		<cfargument type="String" name="input" required="true" hint="the input to check"/>
		<cfargument type="String" name="orig" required="false" default="#arguments.input#"
		            hint="A origional input to include in exception messages. This is not included if it is the same as input."/>
		...
		<cfscript>
			...
			for(local.i = 1; local.i <= arrayLen(instance.blacklistPatterns); local.i++) {
				local.p = instance.blacklistPatterns[local.i];
				if(local.p.matcher(arguments.input).matches()) {
					NullSafe = newJava("org.owasp.esapi.util.NullSafe");
	...
	<cffunction access="private" returntype="String" name="checkLength" output="false"
	            hint="checks input lengths">
	...
	<cffunction access="private" returntype="String" name="checkEmpty" output="false"
	            hint="checks input emptiness">
		<cfargument type="String" name="context" required="true"/>
		<cfargument type="String" name="input" required="true"/>
		<cfargument type="String" name="orig" required="false" default="#arguments.input#"/>
