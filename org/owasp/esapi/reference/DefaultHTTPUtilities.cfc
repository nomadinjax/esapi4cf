...
<cfcomponent implements="org.owasp.esapi.HTTPUtilities" extends="org.owasp.esapi.util.Object" output="false" hint="Reference implementation of the HTTPUtilities interface. This implementation uses the Apache Commons FileUploader library, which in turn uses the Apache Commons IO library. To simplify the interface, this class uses the current request and response that are tracked by ThreadLocal variables in the Authenticator. This means that you must have called ESAPI.authenticator().setCurrentHTTP(request, response) before calling these methods. Typically, this is done by calling the Authenticator.login() method, which calls setCurrentHTTP() automatically. However if you want to use these methods in another application, you should explicitly call setCurrentHTTP() in your own code.">
	...
	<cffunction access="public" returntype="void" name="assertSecureRequest" output="false" hint="Verifies that the request is 'secure' by checking that the method is a POST and that SSL has been used.  The POST ensures that the data does not end up in bookmarks, web logs, referer headers, and other exposed sources.  The SSL ensures that data has not been exposed in transit.">
		<cfargument required="true" type="org.owasp.esapi.util.HttpServletRequest" name="httpRequest">
		<cfscript>
			// CF8 requires 'var' at the top
			var receivedMethod = "";
	...
	<cffunction access="public" returntype="void" name="verifyCSRFToken" output="false" hint="This implementation uses the parameter name to store the token. This makes the CSRF token a bit harder to search for in an XSS attack.">
		<cfargument required="true" type="org.owasp.esapi.util.HttpServletRequest" name="httpRequest">
		<cfscript>
			var user = variables.ESAPI.authenticator().getCurrentUser();
	...
	<cffunction access="public" returntype="Array" name="getSafeFileUploads" output="false" hint="Uses the Apache Commons FileUploader to parse the multipart HTTP request and extract any files therein. Note that the progress of any uploads is put into a session attribute, where it can be retrieved with a simple JSP.">
		<cfargument required="true" type="org.owasp.esapi.util.HttpServletRequest" name="httpRequest">
	...
	<cffunction access="public" returntype="boolean" name="isSecureChannel" output="false" hint="Returns true if the request was transmitted over an SSL enabled connection. This implementation ignores the built-in isSecure() method and uses the URL to determine if the request was transmitted over SSL.">
		<cfargument required="true" type="org.owasp.esapi.util.HttpServletRequest" name="httpRequest">
		<cfscript>
	...
	<cffunction access="public" returntype="void" name="safeSendForward" output="false" hint="This implementation simply checks to make sure that the forward location starts with 'WEB-INF' and is intended for use in frameworks that forward to JSP files inside the WEB-INF folder.">
		<cfargument required="true" type="org.owasp.esapi.util.HttpServletRequest" name="httpRequest">
		<cfargument required="true" type="org.owasp.esapi.util.HttpServletResponse" name="httpResponse">
