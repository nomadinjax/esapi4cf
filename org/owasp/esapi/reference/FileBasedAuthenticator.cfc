...
<cfcomponent implements="org.owasp.esapi.Authenticator" extends="org.owasp.esapi.util.Object" output="false" hint="Reference implementation of the Authenticator interface. This reference implementation is backed by a simple text file that contains serialized information about users. Many organizations will want to create their own implementation of the methods provided in the Authenticator interface backed by their own user repository. This reference implementation captures information about users in a simple text file format that contains user information separated by the pipe '|' character.">
	...
	<cffunction access="public" returntype="void" name="setHashedPassword" output="false" hint="Add a hash to a User's hashed password list.  This method is used to store a user's old password hashes to be sure that any new passwords are not too similar to old passwords.">
	...
	<cfscript>
		...
		<cffunction access="public" returntype="org.owasp.esapi.User" name="getCurrentUser" output="false" hint="Returns the currently logged user as set by the setCurrentUser() methods. Must not log in this method because the logger calls getCurrentUser() and this could cause a loop.">
			<cfscript>
		...
		<cffunction access="public" returntype="void" name="loadUsersIfNecessary" output="false" hint="Load users if they haven't been loaded in a while.">
			<cfscript>
			// CF8 requires 'var' at the top
			var timestamp = "";
		...
		<cffunction access="public" returntype="void" name="loadUsersImmediately" output="false" hint="file was touched so reload it">
			<cfscript>
			// CF8 requires 'var' at the top
			var reader = "";
			var map = "";
		...
		<cffunction access="private" returntype="org.owasp.esapi.User" name="loginWithUsernameAndPassword" output="false" hint="Utility method to extract credentials and verify them.">
	...
	<cffunction access="public" returntype="void" name="saveUsers" output="false" hint="Save users.">
		...
		<cffunction access="public" returntype="org.owasp.esapi.User" name="login" output="false">
			...
			// if we have a user, verify we are on SSL (POST not required)
				...
				try {
					variables.ESAPI.httpUtilities().isSecureChannel( arguments.httpRequest );
				}
				catch(org.owasp.esapi.errors.AccessControlException e) {
					throwException( createObject( "component", "org.owasp.esapi.errors.AuthenticationException").init( variables.ESAPI, "Attempt to access secure content with an insecure request", e.detail, e ) );
				}
			}
