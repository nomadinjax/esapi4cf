...
<cfcomponent extends="cfesapi.org.owasp.esapi.lang.Object" output="false">
	...
	<cffunction access="public" returntype="PolicyDTO" name="load" output="false">
		...
		<cfscript>
			local.policyDTO = newComponent("cfesapi.org.owasp.esapi.reference.accesscontrol.policyloader.PolicyDTO").init(instance.ESAPI);
			local.config = "";
			...
			try {
				...
				if(local.file == "") {
					throwError(newComponent("cfesapi.org.owasp.esapi.errors.AccessControlException").init(instance.ESAPI, "Unable to load configuration file for the following: " & "ESAPI-AccessControlPolicy.xml", "", cex));
				}
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.AccessControlException").init(instance.ESAPI, "Unable to load configuration file from the following location: " & local.file.getAbsolutePath(), "", cex));
			}
			...
			try {
				...
				for(local.currentRule = 0; local.currentRule < local.numberOfRules; local.currentRule++) {
					...
					try {
						instance.logger.trace(newJava("org.owasp.esapi.Logger").EVENT_SUCCESS, "rulePolicyParameters: " & local.rulePolicyParameter.toStringESAPI());
					}
					/* for Adobe */
					catch(Object e) {
				...
				instance.logger.info(newJava("org.owasp.esapi.Logger").EVENT_SUCCESS, "policyDTO loaded: " & local.policyDTO.toStringESAPI());
			}
			catch(cfesapi.org.owasp.esapi.errors.AccessControlException e) {
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.AccessControlException").init(instance.ESAPI, "Unable to load AccessControlRule parameter. " & " Rule number: " & local.currentRule & " Probably: Rule.name: " & local.ruleName & " Probably: Rule.class: " & local.ruleClass & " " & e.message, "", e));
			}
	...
	<cffunction access="private" returntype="any" name="getPolicyParameter" output="false">
		...
		<cfscript>
			...
			if(!structKeyExists(local, "parametersLoaderClassName") || "" == local.parametersLoaderClassName) {
			...
			local.acrParamaterLoader = newComponent(local.parametersLoaderClassName).init(instance.ESAPI);
