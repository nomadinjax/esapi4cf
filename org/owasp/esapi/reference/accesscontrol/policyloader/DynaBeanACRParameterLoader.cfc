...
<cfcomponent extends="cfesapi.org.owasp.esapi.lang.Object" implements="ACRParameterLoader" output="false">
	...
	<cffunction access="public" returntype="ACRParameterLoader" name="init" output="false">
	...
	<cffunction access="public" returntype="any" name="getParameters" output="false"
	            hint="cfesapi.org.owasp.esapi.reference.accesscontrol.DynaBeanACRParameter">
		...
		<cfscript>
			...
			for(local.currentParameter = 0; local.currentParameter < local.numberOfParameters; local.currentParameter++) {
				...
				local.parameterValue = newComponent("cfesapi.org.owasp.esapi.reference.accesscontrol.policyloader.ACRParameterLoaderHelper").init().getParameterValue(config, currentRule, local.currentParameter, local.parameterType);
			...
			instance.logger.info(newJava("org.owasp.esapi.Logger").SECURITY_SUCCESS, "Loaded " & local.numberOfParameters & " parameters: " & local.policyParameter.toStringESAPI());
