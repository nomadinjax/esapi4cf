	...
	<cffunction access="public" returntype="void" name="addAccessControlRule" output="false">
		...
		<cfscript>
			...
			if(structKeyExists(local, "rule")) {
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.AccessControlException").init(instance.ESAPI, "Duplicate keys are not allowed. " & "Key: " & arguments.key, ""));
			}
			try {
				local.accessControlRule = newComponent(arguments.accessControlRuleClassName).init(instance.ESAPI);
				...
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.AccessControlException").init(instance.ESAPI, 'Unable to create Access Control Rule for key: "' & arguments.key & '" with policyParameters: "' & arguments.policyParameter.toStringESAPI() & '"', "", e));
			}
		</cfscript>
