...
<cfcomponent extends="BaseACR" output="false">
	...
	<cffunction access="private" returntype="Array" name="getParameters" output="false"
	            hint="Convert an array of fully qualified class names into an array of Class objects">
	...
	<cffunction access="private" returntype="any" name="getClassName" output="false"
	            hint="Convert a single fully qualified class name into a Class object">
		...
		<cfscript>
			try {
				if(listFirst(arguments.className, ".") == "cfesapi") {
					local.theClass = newComponent(arguments.className).init(ESAPI=instance.ESAPI);
				}
				else {
	...
	<cffunction access="public" returntype="boolean" name="isAuthorized" output="false"
	            hint="Delegates to the method specified in setPolicyParameters">
