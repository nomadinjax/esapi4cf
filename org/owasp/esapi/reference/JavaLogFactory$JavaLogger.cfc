...
<cfcomponent implements="org.owasp.esapi.Logger" extends="org.owasp.esapi.util.Object" output="false" hint="Reference implementation of the Logger interface. It implements most of the recommendations defined in the Logger interface description. It does not filter out any sensitive data specific to the current application or organization, such as credit cards, social security numbers, etc.  ">
	...
	<cffunction access="public" returntype="JavaLogFactory$JavaLogger" name="init" output="false" hint="Public constructor should only ever be called via the appropriate LogFactory">
		...
		<cfscript>
			...
			variables.currentLevel = convertESAPILeveltoLoggerLevel( ESAPI.securityConfiguration().getLogLevel() );
	...
	<cffunction access="public" returntype="void" name="setLevel" output="false" hint="Note: In this implementation, this change is not persistent, meaning that if the application is restarted, the log level will revert to the level defined in the SAPI SecurityConfiguration properties file.">
	...
	<cffunction access="private" name="convertESAPILeveltoLoggerLevel" output="false" hint="Converts the ESAPI logging level (a number) into the levels used by Java's logger.">
		...
		<cfscript>
			...
			switch(arguments.level) {
				...
				default: {
					throwException( newJava( "java.lang.IllegalArgumentException" ).init( "Invalid logging level. Value was: " & arguments.level ) );
				}
			}
