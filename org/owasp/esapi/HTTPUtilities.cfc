...
<cfinterface hint="The HTTPUtilities interface is a collection of methods that provide additional security related to HTTP requests, responses, sessions, cookies, headers, and logging.">
	...
	<cffunction access="public" returntype="void" name="assertSecureRequest" output="false" hint="Ensures that the current request uses SSL and POST to protect any sensitive parameters in the querystring from being sniffed or logged. For example, this method should be called from any method that uses sensitive data from a web form. This method uses {@link HTTPUtilities##getCurrentRequest()} to obtain the current {@link HttpServletRequest} object">
    ...
    <cffunction access="public" returntype="void" name="setSafeContentType" output="false" hint="Set the content type character encoding header on every HttpServletResponse in order to limit the ways in which the input data can be represented. This prevents malicious users from using encoding and multi-byte escape sequences to bypass input validation routines. Implementations of this method should set the content type header to a safe value for your environment. The default is text/html; charset=UTF-8 character encoding, which is the default in early versions of HTML and HTTP. See RFC 2047 (http://ds.internic.net/rfc/rfc2045.txt) for more information about character encoding and MIME. The DefaultHTTPUtilities reference implementation sets the content type as specified.">
		<cfargument required="true" type="esapi4cf.org.owasp.esapi.HttpServletResponse" name="response" hint="The servlet response to set the content type for.">
	</cffunction>

    <cffunction access="public" returntype="void" name="setNoCacheHeaders" output="false" hint="Set headers to protect sensitive information against being cached in the browser. Developers should make this call for any HTTP responses that contain any sensitive data that should not be cached within the browser or any intermediate proxies or caches. Implementations should set headers for the expected browsers. The safest approach is to set all relevant headers to their most restrictive setting. These include: Cache-Control: no-store; Cache-Control: no-cache; Cache-Control: must-revalidate; Expires: -1. Note that the header 'pragma: no-cache' is only useful in HTTP requests, not HTTP responses. So even though there are many articles recommending the use of this header, it is not helpful for preventing browser caching. This method uses {@link HTTPUtilities##getCurrentResponse()} to obtain the {@link HttpServletResponse} object">
		<cfargument required="true" type="esapi4cf.org.owasp.esapi.HttpServletResponse" name="response">
	</cffunction>
    ...
    <cffunction access="public" returntype="void" name="setCurrentHTTP" output="false" hint="Stores the current HttpRequest and HttpResponse so that they may be readily accessed throughout ESAPI (and elsewhere)">
		<cfargument required="true" name="request" hint="the current request">
		<cfargument required="true" name="response" hint="the current response">
	</cffunction>

    <cffunction access="public" returntype="esapi4cf.org.owasp.esapi.filters.SafeRequest" name="getCurrentRequest" output="false" hint="Retrieves the current HttpServletRequest">
	</cffunction>

    <cffunction access="public" returntype="esapi4cf.org.owasp.esapi.filters.SafeResponse" name="getCurrentResponse" output="false" hint="Retrieves the current HttpServletResponse">
	</cffunction>
    ...
    <cffunction access="public" returntype="void" name="logHTTPRequest" output="false" hint="Format the Source IP address, URL, URL parameters, and all form parameters into a string suitable for the log file. The list of parameters to obfuscate should be specified in order to prevent sensitive information from being logged. If a null list is provided, then all parameters will be logged. If HTTP request logging is done in a central place, the parameterNamesToObfuscate could be made a configuration parameter. We include it here in case different parts of the application need to obfuscate different parameters. This method uses {@link HTTPUtilities##getCurrentResponse()} to obtain the {@link HttpServletResponse} object">
		<cfargument required="true" type="esapi4cf.org.owasp.esapi.HttpServletRequest" name="request">
		<cfargument required="true" type="esapi4cf.org.owasp.esapi.Logger" name="logger" hint="the logger to write the request to">
