...
<cfcomponent extends="cfesapi.org.owasp.esapi.lang.Object" output="false" hint="Class to provide some convenience methods for encryption, decryption, etc.">
	...
	<cffunction access="public" returntype="any" name="generateSecretKeyESAPI" output="false"
	            hint="javax.crypto.SecretKey: Generate a random secret key appropriate to the specified cipher algorithm and key size.">
		...
		<cfscript>
			...
			try {
				...
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.EncryptionException").init(instance.ESAPI, "Failed to generate random secret key", "Failed to generate secret key for " & arguments.alg & " with size of " & arguments.keySize & " bits.", e));
			}
		</cfscript>

	</cffunction>

	<cffunction access="public" returntype="any" name="computeDerivedKey" output="false"
	            hint="javax.crypto.SecretKey: Compute a derived key from the keyDerivationKey for either encryption / decryption or for authentication.">
		...
		<cfscript>
			assert(structKeyExists(arguments, "keyDerivationKey"), "Master key cannot be null.");
			...
			assert(structKeyExists(arguments, "purpose"));
			...
			try {
				...
				throwError(newComponent("cfesapi.org.owasp.esapi.errors.EncryptionException").init(instance.ESAPI, "Encryption failure (internal encoding error: UTF-8)", "UTF-8 encoding is NOT supported as a standard byte encoding: " & e.message, e));
			}
	...
	<cffunction access="public" returntype="boolean" name="isCombinedCipherMode" output="false"
	            hint="Return true if specified cipher mode is one of those specified in the ESAPI.properties file that supports both confidentiality AND authenticity (i.e., a 'combined cipher mode' as NIST refers to it).">
		...
		<cfscript>
			assert(structKeyExists(arguments, "cipherMode"), "Cipher mode may not be null");
			...
			if(arrayFind(local.combinedCipherModes, arguments.cipherMode)) {
				return true;
			}
			else {
				return false;
	...
	<cffunction access="public" returntype="boolean" name="isAllowedCipherMode" output="false"
	            hint="Return true if specified cipher mode is one that may be used for encryption / decryption operations via org.owasp.esapi.Encryptor.">
		<cfargument type="String" name="cipherMode" required="true" hint="The specified cipher mode to be used for the encryption or decryption operation."/>
		...
		<cfscript>
			...
			if(arrayFind(local.extraCipherModes, arguments.cipherMode)) {
				return true;
			}
			else {
				return false;
	...
	<cffunction access="public" returntype="boolean" name="isCipherTextMACvalid" output="false"
	            hint="If a Message Authentication Code (MAC) is required for the specified CipherText object, then attempt to validate the MAC that should be embedded within the CipherText object by using a derived key based on the specified SecretKey.">
	...
	<cffunction access="public" returntype="void" name="overwrite" output="false"
	            hint="Overwrite a byte array with a specified byte. This is frequently done to a plaintext byte array so the sensitive data is not lying around exposed in memory.">
		<cfargument type="binary" name="bytes" required="true" hint="The byte array to be overwritten."/>
		<cfargument type="String" name="x" required="false" default="*"
		            hint="The byte array bytes is overwritten with this byte."/>
	...
	<cffunction access="public" returntype="void" name="copyByteArray" output="false"
	            hint="These provide for a bit more type safety when copying bytes around.">
		...
		<cfargument type="numeric" name="length" required="false" default="#arrayLen(arguments.src)#"
		            hint="the number of array elements to be copied."/>
	...
	<cffunction access="public" returntype="boolean" name="arrayCompare" output="false"
	            hint="A 'safe' array comparison that is not vulnerable to side-channel 'timing attacks'. All comparisons of non-null, equal length bytes should take same amount of time. We use this for cryptographic comparisons.">
		...
		<cfscript>
			...
			if(!structKeyExists(arguments, "b1") || !structKeyExists(arguments, "b2")) {
