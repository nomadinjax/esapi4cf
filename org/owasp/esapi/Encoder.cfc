 ...
 * For the encoding methods, this means that all characters should be encoded, except for a specific
 list of
 * "immune" characters that are known to be safe. For the decoding methods, all encoded characters
should be
 * decoded and if any doubly encoded characters (using the same encoding scheme or two different
encoding schemes)
 * should be rejected.
 *
