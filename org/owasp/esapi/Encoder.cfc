...
interface {
	 ...
	 */
	public string function canonicalize(required string input, boolean restrictMultiple, boolean restrictMixed);
     ...
     * 2)      If an OS call cannot be avoided, then it is recommended that the program to be invoked be invoked directly (e.g., variables.System.exec("nameofcommand" + "parameterstocommand");) as this avoids the use of the command shell. The "parameterstocommand" should of course be validated before passing them to the OS command.
     ...
     *
     * An example use of this method would be: variables.System.exec("dir " + ESAPI.encodeForOS(WindowsCodec, "parameter(s)tocommandwithuserinput");
     *
     * @param codec
