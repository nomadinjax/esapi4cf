...
component implements="org.owasp.esapi.Logger" extends="org.owasp.esapi.util.Object" {
	...
	private string function convertJTypetoCFType(required type) {
		if (arguments.type.isEquals(this.SECURITY_SUCCESS)) {
			return "INFORMATION";
		}
		else if (arguments.type.isEquals(this.SECURITY_FAILURE)) {
			return "WARNING";
		}
		else if (arguments.type.isEquals(this.SECURITY_AUDIT)) {
			return "INFORMATION";
		}
		else if (arguments.type.isEquals(this.EVENT_SUCCESS)) {
			return "INFORMATION";
		}
		else if (arguments.type.isEquals(this.EVENT_FAILURE)) {
			return "WARNING";
		}
		else if (arguments.type.isEquals(this.EVENT_UNSPECIFIED)) {
			return "INFORMATION";
		}
	}
