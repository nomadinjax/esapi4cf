...
component extends="org.owasp.esapi.util.RuntimeException" {
     ...
     */
	public EnterpriseSecurityRuntimeException function init(required org.owasp.esapi.ESAPI ESAPI, required string userMessage, required string logMessage, cause) {
		variables.ESAPI = arguments.ESAPI;
		variables.logger = variables.ESAPI.getLogger(getMetaData(this).name);

		if (structKeyExists(arguments, "cause")) {
			super.init(arguments.userMessage, cause);
		}
		else {
