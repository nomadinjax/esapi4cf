...
<cfcomponent displayname="EnterpriseSecurityRuntimeException" extends="cfesapi.org.owasp.esapi.lang.RuntimeException" output="false" hint="EnterpriseSecurityRuntimeException is the base class for all security related runtime exceptions. You should pass in the root cause exception wherever possible. Constructors for classes extending this class should be sure to call the appropriate super() method in order to ensure that logging and intrusion detection occur properly. All EnterpriseSecurityRuntimeExceptions have two messages, one for the user and one for the log file. This way, a message can be shown to the user that doesn't contain sensitive information or unnecessary implementation details. Meanwhile, all the critical information can be included in the exception so that it gets logged. Note that the 'logMessage' for ALL EnterpriseSecurityRuntimeExceptions is logged in the log file. This feature should be used extensively throughout ESAPI implementations and the result is a fairly complete set of security log records. ALL EnterpriseSecurityRuntimeExceptions are also sent to the IntrusionDetector for use in detecting anomalous patterns of application usage.">
	...
	<cffunction access="public" returntype="String" name="getUserMessage" output="false"
	            hint="Returns message meant for display to users. Note that if you are unsure of what set this message, it would probably be a good idea to encode this message before displaying it to the end user.">
	...
	<cffunction access="public" returntype="String" name="getLogMessage" output="false"
	            hint="Returns a message that is safe to display in logs, but may contain sensitive information and therefore probably should not be displayed to users.">
