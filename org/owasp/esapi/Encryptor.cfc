...
<cfinterface hint="The Encryptor interface provides a set of methods for performing common encryption, random number, and hashing operations. Implementations should rely on a strong cryptographic implementation, such as JCE or BouncyCastle. Implementors should take care to ensure that they initialize their implementation with a strong 'master key', and that they protect this secret as much as possible.">
	...
	<cffunction access="public" returntype="any" name="encryptESAPI" output="false"
	            hint="Encrypts the provided plaintext bytes using the cipher transformation specified by the property Encryptor.CipherTransformation as defined in the ESAPI.properties file and the specified secret key. This method is similar to ##encrypt(PlainText) except that it permits a specific SecretKey to be used for encryption.">
	...
	<cffunction access="public" returntype="any" name="decryptESAPI" output="false"
	            hint="Decrypts the provided CipherText using the information from it and the specified secret key. This decrypt method is similar to ##decrypt(CipherText) except that it allows decrypting with a secret key other than the master secret key.">
		<cfargument type="any" name="key" required="false" hint="The SecretKey to use for encrypting the plaintext."/>
	...
	<cffunction access="public" returntype="boolean" name="verifySignature" output="false"
	            hint="Verifies a digital signature (created with the sign method) and returns the boolean result. 'Limitations:' A new public/private key pair used for ESAPI 2.0 digital signatures with this method and ##sign(String) are dynamically created when the default reference implementation class, JavaEncryptor is first created. Because this key pair is not persisted nor is the public key shared, this method and the corresponding ##sign(String) can not be used with expected results across JVM instances. This limitation will be addressed in ESAPI 2.1.">
