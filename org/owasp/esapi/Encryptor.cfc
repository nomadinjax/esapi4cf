...
<cfinterface hint="The Encryptor interface provides a set of methods for performing common encryption, random number, and hashing operations. Implementations should rely on a strong cryptographic implementation, such as JCE or BouncyCastle. Implementors should take care to ensure that they initialize their implementation with a strong 'master key', and that they protect this secret as much as possible.">
	...
	<cffunction access="public" returntype="String" name="encryptString" output="false" hint="Encrypts the provided plaintext and returns a ciphertext string. @deprecated Why this method is deprecated? Most cryptographers strongly suggest that if you are creating crypto functionality for general-purpose use, at a minimum you should ensure that it provides authenticity, integrity, and confidentiality. This method only provides confidentiality, but not authenticity or integrity. Therefore, you are encouraged to use one of the other encryption methods referenced below. Because this method provides neither authenticity nor integrity, it may be removed in some future ESAPI Java release. Note: there are some cases where authenticity / integrity are not that important. For instance, consider a case where the encrypted data is never out of your application's control. For example, if you receive data that your application is encrypting itself and then storing the encrypted data in its own database for later use (and no other applications can query or update that column of the database), providing confidentiality alone might be sufficient. However, if there are cases where your application will be sending or receiving already encrypted data over an insecure, unauthenticated channel, in such cases authenticity and integrity of the encrypted data likely is important.">
		<cfargument required="true" type="String" name="plaintext" hint="the plaintext String to encrypt">
	</cffunction>


	<cffunction access="public" returntype="String" name="decryptString" output="false" hint="Decrypts the provided ciphertext string (encrypted with the encrypt method) and returns a plaintext string. @deprecated Why this method is deprecated? Most cryptographers strongly suggest that if you are creating crypto functionality for general-purpose use, at a minimum you should ensure that it provides authenticity, integrity, and confidentiality. This method only provides confidentiality, but not authenticity or integrity. Therefore, you are encouraged to use one of the other encryption methods referenced below. Because this method provides neither authenticity nor integrity, it may be removed in some future ESAPI Java release. Note: there are some cases where authenticity / integrity are not that important. For instance, consider a case where the encrypted data is never out of your application's control. For example, if you receive data that your application is encrypting itself and then storing the encrypted data in its own database for later use (and no other applications can query or update that column of the database), providing confidentiality alone might be sufficient. However, if there are cases where your application will be sending or receiving already encrypted data over an insecure, unauthenticated channel, in such cases authenticity and integrity of the encrypted data likely is important.">
		<cfargument required="true" type="String" name="ciphertext" hint="the ciphertext (encrypted plaintext)">
	</cffunction>


	<cffunction access="public" returntype="String" name="sign" output="false" hint="Create a digital signature for the provided data and return it in a string.">
		<cfargument required="true" type="String" name="data" hint="the data to sign">
	</cffunction>


	<cffunction access="public" returntype="boolean" name="verifySignature" output="false" hint="Verifies a digital signature (created with the sign method) and returns the boolean result.">
